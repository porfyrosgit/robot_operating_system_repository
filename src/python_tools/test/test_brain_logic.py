...
def generate_test_description():
    # This launches the C++ brain node before the test runs.
    # Without this, nothing is listening on /imu_mock and nothing
...
def test_brain_stabilization_logic():
    ...
    assert 'STABILIZE' in captured_commands, \
        f'TEST FAILED: brain did not stabilize for tilt 0.8 Received: {captured_commands}'
    ...
    # The Recovery Execution Loop
    # We keep telling the Brain "You are level" until it acknowledges it.
    # BUG FIX: the original loop was asserting immediately without ever publishing
    # safe_msg or calling spin_once, so the brain never received the safe da